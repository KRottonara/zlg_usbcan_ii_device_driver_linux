...
if __name__ == "__main__":
    ...
    # Pack speed_ctrl as 4 bytes little-endian
    speed_bytes = speed_ctrl.to_bytes(4, byteorder='little', signed=True)
    msg.Data[0] = 0xA2
    msg.Data[1] = max_torque
    msg.Data[2] = 0x00
    msg.Data[3] = 0x00
    msg.Data[4] = speed_bytes[0]
    msg.Data[5] = speed_bytes[1]
    msg.Data[6] = speed_bytes[2]
    msg.Data[7] = speed_bytes[3]

    for ch in range(MAX_CHANNELS):
        send_ret = lib.VCI_Transmit(DevType, 0, ch, byref(msg), 1)
        if send_ret == 1:
            print(f"Speed command sent to motor ID {motor_id} on channel {ch}")
        else:
